    ...
    while (maxSize > 0)
    {
        ...
        while ((chunkIdx < _chunks.count()) && chunksLoopOngoing)
        {
            ...
            else
            {
                ...
                if (count > 0)
                {
                    buffer += chunk.data.mid((int)chunkOfs, (int)count);//Modify by lbg 20171206:修改静态问题
                    maxSize -= count;
                    pos += count;
                    if (highlighted)
                        *highlighted += chunk.dataChanged.mid((int)chunkOfs, (int)count);//Modify by lbg 20171205:修改静态问题
                }
            }
        }
...
bool Chunks::insert(qint64 pos, char b)
{
    ...
    qint64 posInBa = pos - _chunks[chunkIdx].absPos;
    _chunks[chunkIdx].data.insert((int)posInBa, b);//Modify by lbg 20171205:修改静态问题
    _chunks[chunkIdx].dataChanged.insert((int)posInBa, char(1));//Modify by lbg 20171205:修改静态问题
...
bool Chunks::removeAt(qint64 pos)
{
    ...
    int chunkIdx = getChunkIndex(pos);
    qint64 posInBa = pos - _chunks[chunkIdx].absPos;
    _chunks[chunkIdx].data.remove((int)posInBa, 1);//Modify by lbg 20171205:修改静态问题
    _chunks[chunkIdx].dataChanged.remove((int)posInBa, 1);//Modify by lbg 20171205:修改静态问题
    for (int idx=chunkIdx+1; idx < _chunks.size(); idx++)
        _chunks[idx].absPos -= 1;
    _size -= 1;
    _pos = pos;
...
{
     // Modify by jq 2017-12-2 静态测试
    QByteArray buffer = data(pos, 1);
