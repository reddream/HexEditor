...
class QHEXEDIT_API QHexEdit : public QAbstractScrollArea
{
    ...
    //组模式的设置，默认为false
    Q_PROPERTY(bool groupMode READ groupMode WRITE setGroupMode)
    /* 设置每组显示得字节数 */
    Q_PROPERTY(unsigned int groupSize READ groupSize WRITE setGroupSize)
    /* 每组之间的分隔符 */
    Q_PROPERTY(char separated READ getSeparate WRITE setSeparate)
...
public:
    ...
    */
    ...
    /* 返回干扰技术码 */
    QString toJAMCodeString(int groupSize=4,QString strSplit="-");


    /* 查找到某位置 */
    void findPos(unsigned int pos,bool isFindInByte = true);
...
public:
    ...
    //设置几个字节为一组得显示，默认为4，只有在groupMode 为true 才生效
    void setGroupSize(unsigned int groupSize = 4);
    //几个字节为一组显示。
    unsigned int groupSize() const;

    void setSeparate(char split='-');
    char getSeparate() const;

    //设置组模式
    void setGroupMode(bool flagGroupMode);
    const bool groupMode();
...
protected:
    ...
    //以四个字节为一组绘制
   void paintHexInGroupMode(QPaintEvent *event);
...
//    qint64 calculatePos(qint64 pos);
    // Private utility functions
    void init();
    void readBuffers();
    QString toReadable(const QByteArray &ba);

    //JAMCode Format XXXXXXXX-XXXXXXXX
    //QByteArray to Hex String
    //groupSize is the number of XX spliting by -
    QString toJAMCode(const QByteArray &ba,unsigned short groupSize=4,QString splitStr="-");
...
private:
    ...
    bool m_isGroupMode;
    //每组多少个字节
    unsigned int m_groupBytes;
    //每组多少个字符
    unsigned int m_groupCharLen;
    //每组分隔符
    QString m_separated;
    /*! \endcond docNever */
};
